...
from miniagenticrouter.agents.interactive import InteractiveAgent
from miniagenticrouter.agents.interactive_textual import TextualAgent
...
from miniagenticrouter.run.extra.config import configure_if_first_time
from miniagenticrouter.run.utils.save import save_traj
from miniagenticrouter.utils.log import logger

DEFAULT_CONFIG = Path(os.getenv("MAR_MINI_CONFIG_PATH", builtin_config_dir / "mini.yaml"))
...
def main(
    visual: bool = typer.Option(False, "-v", "--visual", help="Toggle (pager-style) UI (Textual) depending on the MAR_VISUAL_MODE_DEFAULT environment setting",),
    model_name: str | None = typer.Option( None, "-m", "--model", help="Model to use",),
    model_class: str | None = typer.Option(None, "--model-class", help="Model class to use (e.g., 'anthropic' or 'miniagenticrouter.models.anthropic.AnthropicModel')", rich_help_panel="Advanced"),
    ...
    # Both visual flag and the MAR_VISUAL_MODE_DEFAULT flip the mode, so it's essentially a XOR
    agent_class = InteractiveAgent
    if visual == (os.getenv("MAR_VISUAL_MODE_DEFAULT", "false") == "false"):
