...
from miniagenticrouter import global_config_file
...
More information at https://mini-agentic-router.dev/latest/quickstart/
To find the best model, check the leaderboard at https://swebench.com/
"""


def configure_if_first_time():
    if not os.getenv("MAR_CONFIGURED"):
        console.print(Rule())
        setup()
        console.print(Rule())
...
def setup():
    ...
    default_model = prompt(
        ...
        default=os.getenv("MAR_MODEL_NAME", ""),
    ).strip()
    if default_model:
        set_key(global_config_file, "MAR_MODEL_NAME", default_model)
    console.print(
