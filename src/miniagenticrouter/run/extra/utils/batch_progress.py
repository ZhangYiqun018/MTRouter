...
import miniagenticrouter.models
...
class RunBatchProgressManager:
    def __init__(
        ...
        self._model_usage: dict[str, int] = collections.defaultdict(int)
        """Track model usage across all tasks for multi-model routers."""

        # Judge and turns statistics
        self._judge_correct = 0
        self._judge_total = 0
        self._total_turns = 0
        ...
        self._main_task_id = self._main_progress_bar.add_task(
            ...
            stats="[green]✓ 0/0 (0.0%)[/green]  [yellow]avg: 0.0 turns[/yellow]"
        )
    ...
    def update_model_usage(self, model_usage: dict[str, int]) -> None:
        """Update global model usage statistics.

        Args:
            model_usage: Dict mapping model names to call counts for a single task
        """
        with self._lock:
            for model, count in model_usage.items():
                self._model_usage[model] += count
        self.update_exit_status_table()

    def update_judge_result(self, is_correct: bool) -> None:
        """Update Judge statistics.

        Args:
            is_correct: Whether the judge marked the answer as correct
        """
        with self._lock:
            self._judge_total += 1
            if is_correct:
                self._judge_correct += 1
        self.update_exit_status_table()
        self._update_total_costs()

    def update_turns(self, turns: int) -> None:
        """Update turns statistics.

        Args:
            turns: Number of turns (conversation rounds) for this task
        """
        with self._lock:
    ...
    def _get_model_distribution_str(self) -> str:
        """Get a formatted string showing model distribution."""
        ...
        sorted_models = sorted(self._model_usage.items(), key=lambda x: x[1], reverse=True)

        # Format: "gpt-5: 45% (18) | deepseek: 35% (14) | minimax: 20% (8)"
        parts = []
        for model, count in sorted_models:
            # Shorten model name (remove provider prefix)
            short_name = model.split("/")[-1] if "/" in model else model
            pct = count / total * 100
            parts.append(f"{short_name}: {pct:.0f}% ({count})")
    ...
    def update_exit_status_table(self):
        ...
        with self._lock:
            ...
            # Add Judge statistics row (always show)
            judge_rate = 100 * self._judge_correct / self._judge_total if self._judge_total > 0 else 0
            t.add_row(
                "[green]Judge[/green]",
                f"{self._judge_correct}/{self._judge_total}",
                f"[green]{judge_rate:.1f}% correct[/green]",
            )

            # Add average turns row (always show)
            avg_turns = self._total_turns / self._completed_tasks if self._completed_tasks > 0 else 0
            t.add_row(
                "[yellow]Avg Turns[/yellow]",
                f"{avg_turns:.1f}",
                f"[yellow]total: {self._total_turns} across {self._completed_tasks} tasks[/yellow]",
            )

            # Add model distribution row if multi-model
            model_dist = self._get_model_distribution_str()
            if model_dist:
                t.add_row("[magenta]Models[/magenta]", "", f"[magenta]{model_dist}[/magenta]")
    ...
    def _update_total_costs(self) -> None:
        with self._lock:
            ...
            judge_rate = 100 * self._judge_correct / self._judge_total if self._judge_total > 0 else 0
            avg_turns = self._total_turns / self._completed_tasks if self._completed_tasks > 0 else 0
            stats_str = f"[green]✓ {self._judge_correct}/{self._judge_total} ({judge_rate:.1f}%)[/green]  [yellow]avg: {avg_turns:.1f} turns[/yellow]"

            self._main_progress_bar.update(
                self._main_task_id,
                total_cost=f"{miniagenticrouter.models.GLOBAL_MODEL_STATS.cost:.2f}",
