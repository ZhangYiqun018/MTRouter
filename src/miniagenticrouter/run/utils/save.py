...
from miniagenticrouter import Agent, __version__
...
def save_traj(
    ...
    **kwargs,
    ...
    data = {
        ...
        "messages": [],
        "trajectory_format": "mini-agentic-router-1",
    } | kwargs
    if agent is not None:
        ...
        data["info"]["config"] = {
            ...
            "environment": _asdict(agent.env.config) if agent.env else None,
            "agent_type": _get_class_name_with_module(agent),
            "model_type": _get_class_name_with_module(agent.model),
            "environment_type": _get_class_name_with_module(agent.env) if agent.env else None,
        }
    if extra_info:
