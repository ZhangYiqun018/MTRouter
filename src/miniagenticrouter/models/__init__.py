...
# Initialize the model registry to load custom models into litellm
...
class GlobalModelStats:
    ...
    def __init__(self):
        ...
        self.cost_limit = float(os.getenv("MAR_GLOBAL_COST_LIMIT", "0"))
        self.call_limit = int(os.getenv("MAR_GLOBAL_CALL_LIMIT", "0"))
        if (self.cost_limit > 0 or self.call_limit > 0) and not os.getenv("MAR_SILENT_STARTUP"):
...
def get_model(input_model_name: str | None = None, config: dict | None = None) -> Model:
    ...
    if (from_env := os.getenv("MAR_MODEL_API_KEY")) and not str(type(model_class)).endswith("DeterministicModel"):
        config.setdefault("model_kwargs", {})["api_key"] = from_env

    # Only enable cache control for models using Anthropic's native API
    # Check registry first for explicit provider, fall back to name-based detection
    if "set_cache_control" not in config:
        model_info = _registry.get(resolved_model_name)
        if model_info is not None:
            # Use registry provider info - only enable for native Anthropic API
            is_anthropic_native = model_info.provider.lower() == "anthropic"
        else:
            # Fall back to name-based detection for unregistered models
            is_anthropic_native = any(
                s in resolved_model_name.lower() for s in ["anthropic", "sonnet", "opus", "claude"]
            )
...
_MODEL_CLASS_MAPPING = {
    ...
    "anthropic": "miniagenticrouter.models.anthropic.AnthropicModel",
    "litellm": "miniagenticrouter.models.litellm_model.LitellmModel",
    "litellm_response": "miniagenticrouter.models.litellm_response_api_model.LitellmResponseAPIModel",
    "openrouter": "miniagenticrouter.models.openrouter_model.OpenRouterModel",
    "portkey": "miniagenticrouter.models.portkey_model.PortkeyModel",
    "portkey_response": "miniagenticrouter.models.portkey_response_api_model.PortkeyResponseAPIModel",
    "requesty": "miniagenticrouter.models.requesty_model.RequestyModel",
    "deterministic": "miniagenticrouter.models.test_models.DeterministicModel",
    # Routers (new paths)
...
def get_model_class(model_name: str, model_class: str = "") -> type:
    ...
    e.g., "anthropic" or "miniagenticrouter.models.anthropic.AnthropicModel"),
