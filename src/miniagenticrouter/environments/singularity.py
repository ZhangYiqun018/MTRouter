...
class SingularityEnvironmentConfig:
    ...
    executable: str = os.getenv("MAR_SINGULARITY_EXECUTABLE", "singularity")
...
class SingularityEnvironment:
    def __init__(
        ...
        self.logger = logger or logging.getLogger("miniagenticrouter.environment")
    ...
    def _build_sandbox(self) -> Path:
        ...
        for attempt in range(max_retries):
            sandbox_dir = Path(tempfile.gettempdir()) / f"miniagenticrouter-{uuid.uuid4().hex[:8]}"
            try:
                subprocess.run(
