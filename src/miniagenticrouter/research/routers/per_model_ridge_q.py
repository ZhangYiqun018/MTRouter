"""Per-model Ridge Q-function for learned routing.

Each model has its own Ridge regressor:
...
Unlike the shared RidgeQFunction which uses features [z, m(a)],
this approach trains independent weights per model, enabling
the model ranking to vary based on conversation context.
"""
...
class PerModelRidgeQFunction(QFunction):
    """Q-function using per-model Ridge regression for score and cost prediction.

    Each model has its own Ridge regressor trained only on that model's data.
    This enables context-aware model selection by allowing the model ranking
    to depend on the history embedding.

    Advantages over shared Ridge:
    - score_hat(z, a1) - score_hat(z, a2) = (w_{a1} - w_{a2}) · z depends on z
    - Model ranking varies with conversation context
    - More expressive than shared linear model
    ...
    def __init__(
        ...
        """Initialize PerModelRidgeQFunction.
        ...
        # Per-model Ridge models (loaded lazily)
        self.score_models: dict[str, Ridge] | None = None
        self.cost_models: dict[str, Ridge] | None = None
    ...
    def load(
        ...
        """Load per-model Ridge models from directory.

        Args:
            path: Directory containing per_model_ridge.pkl.
        ...
        """
        path = Path(path)
        data = joblib.load(path / "per_model_ridge.pkl")
        ...
        print(f"Loaded per-model Ridge models from {path}")
        print(f"  Models: {list(self.score_models.keys())}")
        print(f"  Encoder dim: {self._encoder_dim}")

    def save(self, path: Path | str) -> None:
        """Save per-model Ridge models to directory.
        ...
        joblib.dump(data, path / "per_model_ridge.pkl")
    ...
    def predict(
        ...
        if self.score_models is None or self.cost_models is None:
            raise ValueError("Per-model Ridge models not loaded. Call load() first.")
        ...
        # Compute Q-value for each model using its own Ridge
        q_values = []
        for model_name in available_models:
            ...
            # Use per-model Ridge regressors
...
@dataclass
class PerModelRidgeInferenceRequest:
...
class BatchedPerModelRidgeQFunction:
    """Thread-safe batched per-model Ridge Q-function inference.

    Collects inference requests from multiple threads and processes them
    in batches to improve GPU utilization for encoding.

    Example:
        >>> ridge = PerModelRidgeQFunction(model_dir="outputs/per_model_ridge")
        >>> batched = BatchedPerModelRidgeQFunction(ridge, batch_size=16)
    ...
    def __init__(
        self,
        ridge_q_func: PerModelRidgeQFunction,
        batch_size: int = 16,
        timeout: float = 0.02,
    ):
        """Initialize BatchedPerModelRidgeQFunction.

        Args:
            ridge_q_func: The underlying PerModelRidgeQFunction.
            batch_size: Maximum batch size before processing.
            timeout: Maximum seconds to wait for batch to fill.
        """
        self.ridge_q_func = ridge_q_func
        self.batch_size = batch_size
        self.timeout = timeout

        self._request_queue: queue.Queue[PerModelRidgeInferenceRequest] = queue.Queue()
        self._shutdown = threading.Event()
        self._worker_thread = threading.Thread(target=self._worker_loop, daemon=True)
        self._worker_thread.start()

        # Ensure encoder is loaded
        self.ridge_q_func._ensure_encoder()
    ...
    def predict(
        ...
        """
        request = PerModelRidgeInferenceRequest(
            messages=history,
    ...
    def _collect_batch(self) -> list[PerModelRidgeInferenceRequest]:
        """Collect requests until batch is full or timeout."""
        batch: list[PerModelRidgeInferenceRequest] = []
    ...
    def _process_batch(self, batch: list[PerModelRidgeInferenceRequest]) -> None:
        """Process a batch of requests with batched encoding.

        Steps:
        1. Batch encode all histories using HistoryEncoder.forward_batch()
        2. For each request, use per-model Ridge to predict
        """
        try:
            with torch.no_grad():
                # Step 1: Batch encode all histories
                all_messages = [req.messages for req in batch]
                embeddings = self.ridge_q_func._history_encoder.forward_batch(all_messages)
    ...
    def _compute_q_for_models(
        ...
        for model_name in available_models:
            canonical_name = self.ridge_q_func._get_canonical_name(model_name)

            if canonical_name is None or canonical_name not in self.ridge_q_func.score_models:
                q_values.append(0.0)
                continue

            score_pred = self.ridge_q_func.score_models[canonical_name].predict(X)[0]
            cost_pred = self.ridge_q_func.cost_models[canonical_name].predict(X)[0]

            q = score_pred - self.ridge_q_func.lambda_ * cost_pred
            q_values.append(q)
    ...
    def set_lambda(self, lambda_: float) -> None:
        """Update the cost penalty coefficient."""
        self.ridge_q_func.set_lambda(lambda_)
    ...
    def load(
        ...
        """Load weights (delegates to underlying ridge_q_func)."""
        self.ridge_q_func.load(path, expected_model_names=expected_model_names)

    def save(self, path) -> None:
        """Save weights (delegates to underlying ridge_q_func)."""
        self.ridge_q_func.save(path)
