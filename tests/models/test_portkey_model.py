...
from miniagenticrouter.models.portkey_model import PortkeyModel, PortkeyModelConfig


def test_portkey_model_missing_api_key():
    """Test that PortkeyModel raises ValueError when no API key is provided."""
    with patch("miniagenticrouter.models.portkey_model.Portkey"):
...
def test_portkey_model_initialization():
    ...
    with patch("miniagenticrouter.models.portkey_model.Portkey", mock_portkey_class):
...
def test_portkey_model_query():
    ...
    with patch("miniagenticrouter.models.portkey_model.Portkey", mock_portkey_class):
        with patch.dict(os.environ, {"PORTKEY_API_KEY": "test-key"}):
            with patch("miniagenticrouter.models.portkey_model.litellm.cost_calculator.completion_cost") as mock_cost:
...
def test_portkey_model_cost_tracking_ignore_errors():
    ...
    with patch("miniagenticrouter.models.portkey_model.Portkey", mock_portkey_class):
        with patch.dict(os.environ, {"PORTKEY_API_KEY": "test-key"}):
            ...
            with patch(
                "miniagenticrouter.models.portkey_model.litellm.cost_calculator.completion_cost",
